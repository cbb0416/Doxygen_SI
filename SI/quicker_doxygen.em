...
macro InsertFileHeaderEN(hbuf, ln,szName,szContent)
{
	...
	InsBufLine(hbuf, ln + 2,  " * \@file    @sz@")
    InsBufLine(hbuf, ln + 3,  " * \@authur  @szName@")
    InsBufLine(hbuf, ln + 4,  " * \@version ")
    //date
    SysTime = GetSysTime(1)
    sz=SysTime.Year
    sz1=SysTime.month
    sz2=SysTime.day
	InsBufLine(hbuf, ln + 5,  " * \@data    @sz@-@sz1@-@sz2@")
    InsBufLine(hbuf, ln + 6,  " * \@brief   ")
    //description
	...
	InsBufLine(hbuf, ln + 7,  " * \@description @szContent@")
...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 0,  "/******************************************************************************")
    InsBufLine(hbuf, ln + 1,  "")
    InsBufLine(hbuf, ln + 2,  "                  版权所有 (C), 2013 上海移位通信技术有限公司")
...
macro FuncHeadCommentEN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    InsBufLine(hbuf, ln+3, " * \@brief   ")
    InsBufLine(hbuf, ln+4, " * \@authur	@szMyName@")
    ...
    if(iIns == 0)
    {       
            InsBufLine(hbuf, ln+1, " * \@param   None")
			ln = ln + 1
    }

    InsBufLine(hbuf, ln+1, " * \@return  @szRet@")
    
    SysTime = GetSysTime(1);
    sz1=SysTime.Year
    sz2=SysTime.month
    sz3=SysTime.day
    InsBufLine(hbuf, ln + 2, " * \@date    @sz1@-@sz2@-@sz3@")
	InsBufLine(hbuf, ln + 3, " * \@see     ")
	InsBufLine(hbuf, ln + 4, " * \@note    ")
    InsBufLine(hbuf, ln + 5, " ******************************************************************************")
	InsBufLine(hbuf, ln + 6, " */")
    if ((newFunc == 1) && (strlen(szFunc)>0))
    {
        InsBufLine(hbuf, ln+7, "UINT32_T  @szFunc@( # )")
        InsBufLine(hbuf, ln+8, "{");
        InsBufLine(hbuf, ln+9, "    #");
    ...
    sel.lnLast = ln + 7        
    szContent = Ask("Description")
    DelBufLine(hbuf,oldln + 3)
    setWndSel(hwnd,sel)
    newln = CommentContent(hbuf,oldln + 3," * \@brief   ",szContent,0) - 2
    ln = ln + newln - oldln
    if ((newFunc == 1) && (strlen(szFunc)>0))
    {
        //提示输入函数返回值名
        ...
        if(strlen(szRet) > 0)
        {
            PutBufLine(hbuf, ln+1, " Return Value : @szRet@")            
            PutBufLine(hbuf, ln+7, "@szRet@ @szFunc@( # )")
            SetbufIns(hbuf,ln+7,strlen(szRet)+strlen(szFunc) + 3
        }
        szFuncDef = ""
        isFirstParam = 1
        sel.ichFirst = strlen(szFunc)+strlen(szRet) + 3
...
macro SelectAll()
{
 ...
 {
  sel.lnLast = ln - 1;
  sel.ichLim  = strlen(GetBufLine (hbuf, sel.lnLast ));
 }
 else
 {
  sel.lnLast = 0;
